...
const PlaygroundModal = {
    ...
    /**
     * Загрузка стилей модального окна
     */
    loadStyles: function() {
        const link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = 'css/pg-modal.css';
        document.head.appendChild(link);
    },
    
    /**
     * Загрузка HTML-структуры модального окна
     */
    loadHTML: async function() {
        try {
            const response = await fetch('pg-modal.html');
            if (!response.ok) {
            ...
            console.error('Ошибка загрузки HTML модального окна:', error);
            
            // Создаем структуру напрямую, если не удалось загрузить файл
            this.createModalDirectly();
    ...
    /**
     * Создание модального окна напрямую, если не удалось загрузить HTML
     */
    createModalDirectly: function() {
        console.log('Создаем структуру модального окна напрямую');
        
        // Создаем backdrop
        const backdrop = document.createElement('div');
        backdrop.id = 'pgModalBackdrop';
        ...
        title.className = 'pg-modal-title';
        title.textContent = 'Площадка';
        modal.appendChild(title);
        
        // Поля ввода
        const inputsContainer = document.createElement('div');
        inputsContainer.className = 'pg-modal-inputs';
        
        // Поле ширины
        const widthContainer = document.createElement('div');
        widthContainer.className = 'pg-input-container';
        const widthInput = document.createElement('input');
        widthInput.type = 'text';
        widthInput.id = 'pgWidthInput';
        widthInput.className = 'pg-input';
        widthInput.value = window.app && window.app.playgroundWidth ? window.app.playgroundWidth : '40';
        widthInput.placeholder = 'Ширина, м';
        widthContainer.appendChild(widthInput);
        inputsContainer.appendChild(widthContainer);
        
        // Поле длины
        const lengthContainer = document.createElement('div');
        lengthContainer.className = 'pg-input-container';
        const lengthInput = document.createElement('input');
        lengthInput.type = 'text';
        lengthInput.id = 'pgLengthInput';
        lengthInput.className = 'pg-input';
        lengthInput.value = window.app && window.app.playgroundLength ? window.app.playgroundLength : '30';
        lengthInput.placeholder = 'Длина, м';
        lengthContainer.appendChild(lengthInput);
        inputsContainer.appendChild(lengthContainer);
        
        modal.appendChild(inputsContainer);
        
        // Надпись "Цвет площадки"
        const colorLabel = document.createElement('div');
        colorLabel.className = 'pg-color-label';
        colorLabel.textContent = 'Цвет площадки:';
        modal.appendChild(colorLabel);
        
        // Цветовые квадраты
        const colorPicker = document.createElement('div');
        colorPicker.className = 'pg-color-picker';
        
        const colors = [
            { color: 'серый', hex: '#D9D9D9' },
            { color: 'черный', hex: '#100F0F' },
            { color: 'зеленый', hex: '#2E7D32' },
            { color: 'коричневый', hex: '#5D4037' }
        ];
        
        // Определяем текущий цвет площадки
        const currentColor = window.app && window.app.playgroundColor ? window.app.playgroundColor : 'зеленый';
        
        colors.forEach(c => {
            const square = document.createElement('div');
            square.className = 'pg-color-square';
            if (c.color === currentColor) square.className += ' selected';
            square.setAttribute('data-color', c.color);
            square.style.backgroundColor = c.hex;
        ...
        applyButton.id = 'pgApplyButton';
        applyButton.className = 'pg-apply-button';
        applyButton.textContent = 'Применить';
        modal.appendChild(applyButton);
        
        // Кнопка закрытия
        const closeButton = document.createElement('button');
        closeButton.id = 'pgCloseButton';
        ...
        console.log('Модальное окно создано напрямую');
    },
